...
describe('Overtricks', () => {
  it('returns 30 above the line for each extra trick made in NT and Major suits', () => {
    ...
    let heartsScore = hearts.scoring('They', 'Hearts', 5, 7)
    let spades = new Scoring()
    let spadesScore = spades.scoring('They', 'Spades', 1, 3)
    expect(NT).toEqual(['We', {below: 130, above: 90}])
    expect(heartsScore).toEqual(['They', {below: 150, above: 60}])
    expect(spadesScore).toEqual(['They', {below: 30, above: 60}])
  });
  it('returns 30 above the line for each extra trick made in NT and Major suits', () => {
    let diamonds = score.scoring('We', 'diamonds', 5, 7)
    let clubs = new Scoring()
    let clubsScore = clubs.scoring('They', 'clubs', 1, 6)
    expect(diamonds).toEqual(['We', {below: 100, above: 40}])
    expect(clubsScore).toEqual(['They', {below: 20, above: 100}])
  });  
});
...
describe('Defeated contract', () => {
  it('returns 50 above the line for the other pair for every undertrick', () => {
    ...
    let hearts = new Scoring()
    let secondScore = hearts.scoring('They', 'Hearts', 7, 3)
