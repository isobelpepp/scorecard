...
describe('Doubled contracts - bid and made', () => {
  ...
  let score = scoring('We', 'NT', 5, 5, 'doubled')
  expect(score[1]).toEqual({'below': 320, 'above': null})
  })
  it('doubles the score for Major suits below the line', () => {
    let heartsScore = scoring('We', 'Hearts', 3, 3, 'doubled')
    let spadesScore = scoring('We', 'Spades', 7, 7, 'doubled')
  ...
  it('doubles the score for Minor suits below the line', () => {
    let diamondsScore = scoring('We', 'Diamonds', 2, 2, 'doubled')
    let clubsScore = scoring('We', 'Clubs', 6, 6, 'doubled')
...
describe('Doubled contracts - underbid, not vulnerable', () => {
  ...
  let score = scoring('They', 'NT', 5, 7, 'doubled')
  expect(score[1]).toEqual({'below': 320, 'above': 200})
  })
  it('doubles the score for Major suits and adds 100 for every overtrick made', () => {
    let heartsScore = scoring('They', 'Hearts', 3, 5, 'doubled')
    let spadesScore = scoring('They', 'Spades', 2, 6, 'doubled')
  ...
  it('doubles the score for Minor suits and adds 100 above for every overtrick', () => {
    let diamondsScore = scoring('They', 'Diamonds', 2, 3, 'doubled')
    let clubsScore = scoring('They', 'Clubs', 6, 7, 'doubled')
